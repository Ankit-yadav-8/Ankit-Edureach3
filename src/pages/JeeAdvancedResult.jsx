...
/* ═══════════════════════════════════════════════════════════
   ...
   Self-contained, scoped under `.jadv-page` so the dark navy
   theme never leaks into the rest of the (light) app.
═══════════════════════════════════════════════════════════ */
      ...
      <div className="wrap">
        ...
        >
          <motion.div
            ...
            animate={{ boxShadow: ["0 0 0px rgba(245,200,66,0)", "0 0 28px rgba(245,200,66,.55)", "0 0 0px rgba(245,200,66,0)"] }}
...
const CSS = `
.jadv-page {
  --navy:#0a0f2e; --deep:#060c26; --gold:#f5c842; --gold2:#e8a800;
  --amber:#ff9f1c; --teal:#00c9b8; --rose:#ff5e7d; --white:#f8f6f0;
  --muted:#a8b0c8; --card:rgba(255,255,255,0.04); --border:rgba(245,200,66,0.18);
...
.jadv-page::before {
  ...
  background:
    radial-gradient(ellipse 80% 60% at 20% 10%, rgba(0,120,180,0.12) 0%, transparent 60%),
    radial-gradient(ellipse 60% 40% at 80% 80%, rgba(100,60,200,0.1) 0%, transparent 60%),
    radial-gradient(ellipse 40% 50% at 60% 30%, rgba(245,200,66,0.05) 0%, transparent 50%);
...
.jadv-page::after {
  ...
  background-image:
    ...
    radial-gradient(1px 1px at 70% 60%, rgba(245,200,66,.5), transparent),
    radial-gradient(1px 1px at 40% 80%, rgba(255,255,255,.45), transparent),
    radial-gradient(1px 1px at 85% 20%, rgba(0,201,184,.45), transparent),
    radial-gradient(1px 1px at 55% 15%, rgba(255,255,255,.4), transparent),
    radial-gradient(1px 1px at 10% 70%, rgba(255,255,255,.4), transparent);
  background-repeat:no-repeat;
  animation:jadvTwinkle 4s ease-in-out infinite alternate;
}
@keyframes jadvTwinkle { from{opacity:.35} to{opacity:.9} }
...
.jadv-back { background:rgba(255,255,255,.05); color:var(--white); }
.jadv-back:hover { background:rgba(255,255,255,.1); transform:translateX(-2px); }
.jadv-share { background:linear-gradient(135deg,var(--gold),var(--amber)); color:var(--navy); border-color:transparent; }
.jadv-share:hover { transform:translateY(-1px); box-shadow:0 6px 20px rgba(245,200,66,.4); }

/* ── header ── */
.jadv-page .header { text-align:center; padding:28px 0 36px; border-bottom:1px solid var(--border); margin-bottom:48px; position:relative; }
.jadv-page .header-badge {
  display:inline-flex; align-items:center; background:linear-gradient(135deg,var(--gold),var(--amber));
  color:var(--navy); font-family:'DM Mono',monospace; font-size:11px; font-weight:500;
...
.jadv-page .header h1 {
  ...
  margin-bottom:10px; background-size:200% auto; animation:jadvShine 6s linear infinite;
}
@keyframes jadvShine { to { background-position:200% center; } }
...
.jadv-page .hstat-val { font-family:'Playfair Display',serif; font-size:2rem; font-weight:700; color:var(--gold); line-height:1; }
...
.jadv-page .section-head h2 { font-family:'Playfair Display',serif; font-size:1.4rem; font-weight:700; color:var(--white); }
.jadv-page .section-head .pill {
  background:var(--border); border:1px solid var(--border); color:var(--gold);
...
.jadv-page .bar { flex:1; height:1px; background:linear-gradient(90deg,var(--border),transparent); }
...
.jadv-page .topper-card { background:var(--card); border:1px solid var(--border); border-radius:16px; padding:28px 24px; position:relative; overflow:hidden; transition:box-shadow .2s; }
.jadv-page .topper-card:hover { box-shadow:0 12px 40px rgba(245,200,66,0.14); }
.jadv-page .topper-card.gold-card { border-color:rgba(245,200,66,0.45); }
.jadv-page .topper-card.gold-card::before { content:''; position:absolute; inset:0; background:radial-gradient(ellipse 70% 60% at 20% 20%, rgba(245,200,66,0.1) 0%, transparent 70%); }
.jadv-page .topper-card.teal-card { border-color:rgba(0,201,184,0.35); }
.jadv-page .topper-card.teal-card::before { content:''; position:absolute; inset:0; background:radial-gradient(ellipse 70% 60% at 80% 20%, rgba(0,201,184,0.08) 0%, transparent 70%); }
.jadv-page .tc-crown { font-size:28px; margin-bottom:8px; line-height:1; }
...
.jadv-page .tc-name { font-family:'Playfair Display',serif; font-size:1.5rem; font-weight:700; color:var(--white); margin-bottom:6px; position:relative; }
...
.jadv-page .tc-marks .big { font-family:'Playfair Display',serif; font-size:2.4rem; font-weight:700; color:var(--gold); line-height:1; }
.jadv-page .tc-marks .of { font-size:13px; color:var(--muted); }
.jadv-page .tc-tag { display:inline-block; background:rgba(245,200,66,0.15); color:var(--gold); font-size:10px; letter-spacing:1px; text-transform:uppercase; padding:3px 10px; border-radius:20px; margin-top:10px; font-family:'DM Mono',monospace; }
.jadv-page .tc-tag.teal { background:rgba(0,201,184,0.12); color:var(--teal); }

/* ── tables ── */
.jadv-page .table-wrap { background:var(--card); border:1px solid var(--border); border-radius:14px; overflow:hidden; }
.jadv-page table { width:100%; border-collapse:collapse; }
.jadv-page thead tr { background:rgba(245,200,66,0.07); border-bottom:1px solid var(--border); }
.jadv-page thead th { text-align:left; padding:14px 18px; font-family:'DM Mono',monospace; font-size:10px; letter-spacing:1.5px; text-transform:uppercase; color:var(--gold); font-weight:500; }
.jadv-page thead th.right { text-align:right; }
.jadv-page tbody tr { border-bottom:1px solid rgba(255,255,255,0.04); transition:background .15s; }
.jadv-page tbody tr:last-child { border-bottom:none; }
.jadv-page tbody tr:hover { background:rgba(255,255,255,0.03); }
.jadv-page tbody td { padding:13px 18px; font-size:14px; color:var(--white); }
.jadv-page tbody td.right { text-align:right; }
.jadv-page .total-row { font-weight:700; background:rgba(245,200,66,0.06); }
.jadv-page .rank-num { display:inline-flex; align-items:center; justify-content:center; width:30px; height:30px; border-radius:8px; font-family:'DM Mono',monospace; font-size:13px; font-weight:500; }
.jadv-page .rank-1 { background:linear-gradient(135deg,#f5c842,#e8a800); color:var(--navy); }
.jadv-page .rank-2 { background:rgba(200,210,220,0.18); color:#c8d2dc; }
.jadv-page .rank-3 { background:rgba(180,120,60,0.22); color:#c8834a; }
.jadv-page .rank-n { background:rgba(255,255,255,0.06); color:var(--muted); }
.jadv-page .name-bold { font-weight:600; letter-spacing:0.2px; }
.jadv-page .zone-tag { display:inline-block; font-size:10px; font-family:'DM Mono',monospace; color:var(--muted); background:rgba(255,255,255,0.05); padding:2px 8px; border-radius:6px; letter-spacing:0.5px; }
.jadv-page .marks-val { font-family:'DM Mono',monospace; font-weight:500; color:var(--teal); }
...
.jadv-page .cat-card { background:var(--card); border:1px solid rgba(255,255,255,0.07); border-radius:12px; padding:16px 18px; display:flex; align-items:center; gap:14px; transition:border-color .2s; }
.jadv-page .cat-card:hover { border-color:var(--border); }
.jadv-page .cat-dot { width:10px; height:10px; border-radius:50%; flex-shrink:0; }
.jadv-page .dot-crl { background:var(--gold); } .jadv-page .dot-ews { background:var(--teal); }
.jadv-page .dot-obc { background:var(--amber); } .jadv-page .dot-sc { background:var(--rose); }
.jadv-page .dot-st { background:#a78bfa; } .jadv-page .dot-pwd { background:#34d399; }
.jadv-page .cat-info { flex:1; } .jadv-page .cat-name { font-size:14px; font-weight:600; color:var(--white); }
...
.jadv-page .badge-crl { background:rgba(245,200,66,0.15); color:var(--gold); }
.jadv-page .badge-ews { background:rgba(0,201,184,0.12); color:var(--teal); }
.jadv-page .badge-obc { background:rgba(255,159,28,0.15); color:var(--amber); }
.jadv-page .badge-sc { background:rgba(255,94,125,0.12); color:var(--rose); }
.jadv-page .badge-st { background:rgba(167,139,250,0.12); color:#a78bfa; }
.jadv-page .badge-pwd { background:rgba(52,211,153,0.12); color:#34d399; }

/* ── female grid ── */
.jadv-page .female-grid { display:grid; grid-template-columns:repeat(auto-fill,minmax(260px,1fr)); gap:12px; }
.jadv-page .female-card { background:var(--card); border:1px solid rgba(255,94,125,0.15); border-radius:12px; padding:14px 16px; display:flex; align-items:center; gap:12px; }
.jadv-page .fc-rank { font-family:'DM Mono',monospace; font-size:18px; font-weight:500; color:var(--rose); min-width:44px; }
.jadv-page .fc-name { font-size:13px; font-weight:600; color:var(--white); }
...
.jadv-page .stat-card { background:var(--card); border:1px solid rgba(255,255,255,0.07); border-radius:12px; padding:20px 18px; text-align:center; }
.jadv-page .stat-card .sv { font-family:'Playfair Display',serif; font-size:2.2rem; font-weight:700; color:var(--gold); line-height:1; margin-bottom:6px; }
.jadv-page .stat-card .sl { font-size:12px; color:var(--muted); letter-spacing:0.5px; }

/* ── footer ── */
.jadv-page .jadv-footer { margin-top:64px; border-top:1px solid var(--border); padding-top:24px; text-align:center; color:var(--muted); font-size:12px; letter-spacing:0.5px; }
.jadv-page .jadv-footer strong { color:var(--gold); }
.jadv-page .jadv-cta {
  display:inline-flex; align-items:center; gap:8px; margin-top:22px;
  background:linear-gradient(135deg,var(--gold),var(--amber)); color:var(--navy);
  border:none; border-radius:50px; padding:12px 26px; font-family:'DM Sans',sans-serif;
  font-size:13px; font-weight:700; cursor:pointer; transition:all .2s;
  box-shadow:0 6px 22px rgba(245,200,66,.3);
}
.jadv-page .jadv-cta:hover { transform:translateY(-2px); box-shadow:0 10px 30px rgba(245,200,66,.45); }
`;
