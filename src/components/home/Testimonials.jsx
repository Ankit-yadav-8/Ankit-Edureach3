    ...
    <section className="section" style={{ background: "linear-gradient(160deg, #fff7f0 0%, #fef3e8 40%, #fff7f0 100%)", position: "relative", overflow: "hidden" }}>
      ...
      <div className="container" style={{ position: "relative", zIndex: 1 }}>
        <div className="title-bar">
          ...
          <h2 className="section-title" style={{ color: "#1a1a2e" }}>What students &amp; parents say</h2>
          <p className="section-sub" style={{ color: "#4b5563" }}>Real outcomes from people who used College Parichay to navigate admissions.</p>
        </div>
        <div className="grid-3">
          {TESTIMONIALS.map((t, i) => (
            <Reveal key={t.name} delay={i * 0.08}>
              <div className="card" style={{ height: "100%", display: "flex", flexDirection: "column", gap: 12, borderTop: `3px solid ${t.accent}`, border: `1px solid ${t.accent}22` }}>
                <Quote size={28} color={t.accent} />
                <p style={{ color: "#374151", lineHeight: 1.65, fontSize: 15, flex: 1 }}>"{t.text}"</p>
                <div style={{ display: "flex", gap: 2 }}>
                  {[1, 2, 3, 4, 5].map((n) => <Star key={n} size={15} fill="#F4A261" color="#F4A261" />)}
                </div>
                <div>
                  <div style={{ fontWeight: 700, color: "#1a1a2e", fontFamily: "Sora" }}>{t.name}</div>
                  <div style={{ fontSize: 12.5, color: "#6b7280" }}>{t.detail}</div>
                </div>
              </div>
            </Reveal>
          ))}
