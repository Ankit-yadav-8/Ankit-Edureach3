  ...
  return (
    <div className="news-ticker-wrap" style={{ marginBottom: 0 }}>
      <div style={{ display: "flex", alignItems: "center", gap: 12, paddingLeft: 16, position: "absolute", left: 0, top: 0, bottom: 0, zIndex: 3 }}>
        <span style={{ background: "#FF693D", color: "#fff", fontSize: 10, fontWeight: 800, padding: "3px 10px", borderRadius: 4, letterSpacing: "1.5px", textTransform: "uppercase", whiteSpace: "nowrap", fontFamily: "'Space Grotesk',sans-serif" }}>
          🔔 LIVE
        </span>
      </div>
        ...
        <div className="news-ticker-track">
          {all.map((n, i) => (
            <span key={i} className="news-ticker-item">
              ...
              <span style={{ color: "#FF693D", fontWeight: 700, marginRight: 4 }}>{n.tags[0]}</span>
              {n.title}
            </span>
          ))}
        </div>
    ...
    <section className="section" id="news" style={{ background: "#ffffff", paddingTop: 0, position: "relative", overflow: "hidden" }}>
      ...
      <div className="container" style={{ paddingTop: "3.5rem" }}>
        <div className="title-bar">
          ...
          <h2 className="section-title" style={{ color: "#1a1a2e" }}>Exam & Admission Updates <span className="accent">That Matter</span></h2>
          <p className="section-sub" style={{ color: "#4b5563" }}>Results, admit cards, counselling schedules — tap any card to read the full story.</p>
        </div>
          ...
          {shown.map((n, i) => (
            <Reveal key={n.slug} delay={(i % 3) * 0.06}>
              <button
                ...
                onMouseEnter={(e) => { e.currentTarget.style.transform = "translateY(-5px)"; e.currentTarget.style.boxShadow = "0 0 0 1.5px rgba(255, 105, 61,.4), 0 16px 48px rgba(255, 105, 61,.15)"; }}
                ...
                <div style={{ height: 155, position: "relative", overflow: "hidden" }}>
                  ...
                  <div style={{ position: "absolute", top: 12, left: 12, display: "flex", gap: 6, flexWrap: "wrap", zIndex: 2 }}>
                    {n.tags.slice(0, 2).map((t) => (
                      <span key={t} style={{
                        background: TAG_COLORS[t] || "#FF693D",
